 ...
 * 
 * Usage:
 *   DATABASE_URL=postgresql://... tsx src/db/apply-migration.ts migrations/0000_create_token_tables.sql
 */
...
async function applyMigration(migrationFile: string) {
  ...
  try {
    ...
    for (const statement of statements) {
      if (statement.trim()) {
        // Remove DO $$ blocks and execute directly
        const cleanStatement = statement.replace(/DO \$\$ BEGIN[\s\S]*?END \$\$;/g, '').trim();
        if (cleanStatement) {
