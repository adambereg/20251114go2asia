...
const getCountriesQuerySchema = z.object({
  ...
  sortBy: z.enum(['id', 'name', 'createdAt', 'citiesCount', 'placesCount']).optional(),
...
app.get('/', async (c) => {
  ...
  try {
    ...
    const { search, sortBy, sortOrder, limit, cursor } = queryParams.data;
    const db = c.get('db');
    ...
    if (cursor) {
      conditions.push(gt(countries.id, cursor));
    }
